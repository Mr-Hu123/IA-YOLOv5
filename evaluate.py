...
exp_folder = os.path.join(args.exp_dir, 'exp_{}'.format(args.exp_num)).replace('\\','/')
...
class YoloTest(object):
    def __init__(self):
        ...
        if args.ISP_FLAG: 
            DIP_model = DIP(self.input_data, self.trainable, self.input_data_clean, self.defog_A, self.IcA)
            self.input_data_adjust = DIP_model.get_adjust_image()
        
        model = YOLOV5(self.input_data_adjust, self.trainable)
        
        self.pred_sbbox, self.pred_mbbox, self.pred_lbbox, self.image_isped, self.isp_params, self.filter_imgs_series = \
            model.pred_sbbox, model.pred_mbbox, model.pred_lbbox, DIP_model.image_isped, DIP_model.filter_params, DIP_model.filter_imgs_series
        ...
        if self.isp_flag:
            ...
            if DefogFilter in cfg.filters:
                for i in range(image_data.shape[0]):
                    dark_i = util.DarkChannel(image_data[i])
                    defog_A_i = util.AtmLight(image_data[i], dark_i)
                    IcA_i = util.DarkIcA(image_data[i], defog_A_i)
                    dark[i, ...] = dark_i
                    defog_A[i, ...] = defog_A_i
                    IcA[i, ...] = IcA_i
    ...
    def evaluate(self):
        ...
        #annotation_path = ./data/dataset_fog/quick_test.txt
        with open(self.annotation_path, 'r') as annotation_file:
            for num, line in enumerate(annotation_file):
                ...
                ground_truth_path = os.path.join(ground_truth_dir_path, str(num) + '.txt').replace('\\','/')
                ...
                predict_result_path = os.path.join(predicted_dir_path, str(num) + '.txt').replace('\\','/')
                t1 = time.time()
